...
def trace(ex: Ex) -> Ex:
	...
	substitute(ex,  Ex(fr'''Tr{{\Gamma^{{{' '.join(lorentz_indices[:D])}}}}} -> {2**(D // 2)} \epsilon^{{{' '.join(lorentz_indices[:D])}}}''', False) )
...
def evaluate(ex: Ex, to_perform_subs: bool = True, to_join_gamma: bool = True) -> Ex:
	...
	while ex not in prior_exs:
		...
		product_rule(ex)
		#print('product_rule', ex)
		spinor_combine(ex)
		#print('spinor_combine', ex)
		evaluate_traces(ex)
		#print('evaluate_traces', ex)
		if to_join_gamma:
			join_gamma(ex)
		distribute(ex)
		#print('distribute', ex)
		...
		#print('unwrap', ex)
		sort_product(ex)
		#print('sort_product', ex)
		sort_sum(ex)
		#print('sort_sum', ex)
		epsilon_to_delta(ex)
		#print('epsilon_to_delta', ex)
		#expand_delta(ex)
		eliminate_kronecker(ex)
		#print('eliminate_kronecker', ex)
		rename_dummies(ex)
		#print('rename_dummies', ex)
		indexbracket_hex(ex)
		#print('indexbracket_hex', ex)
		canonicalise(ex)
		#print('canonicalise', ex)
...
def fierz_expand_2index(ex: Ex, basis: List[Ex], indices: List[str], to_perform_subs: bool = True) -> Ex:
	...
	for element in basis:
		...
		#print('\nrhs', rhs)
		inv_rhs_const = Ex(f'{2**(D // 2 - 1)}/({rhs.top().multiplier/element.top().multiplier})')
		#print('\ninv_rhs_const', inv_rhs_const)

		#print('\npre lhs', inv_rhs_const*element_primed_conj*ex)
